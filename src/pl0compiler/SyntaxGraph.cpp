...
{
/* 0*/ Trans(Trans::Nil,        nullptr,         1, 0, &IRCre::CodeStart),    /*(A)----------->(B)*/
/* 1*/ Trans(Trans::GraphStart, &Graph::s_block, 2, 0, nullptr),              /*(B)---BLOCK--->(C)*/
/* 2*/ Trans(Trans::Symbol,     ".",             3, 0, &IRCre::ProcedureEnd), /*(C)----'.'---->(D)*/
/* 3*/ Trans(Trans::GraphEnd,   nullptr,         0, 0, &IRCre::CodeEnd)       /*(D)----------(END)*/
};

std::array<Trans,21> const SyntaxGraph::s_block =
{
/* 0*/ Trans(Trans::Symbol,     "CONST",                       2,  1, nullptr),                /*(A)--->CONST--->(B)*/
/* 1*/ Trans(Trans::Nil,        nullptr,                       7,  0, nullptr),                /* +------------->(F)*/
/* 2*/ Trans(Trans::Token,      new TType(TType::Identifier),  3,  0, &IRCre::AddSymbol),      /*(B)----ident--->(C)*/
/* 3*/ Trans(Trans::Symbol,     "=",                           4,  0, nullptr),                /*(C)-----'='-----(D)*/
/* 4*/ Trans(Trans::Token,      new TType(TType::Number),      5,  0, &IRCre::AddConstant),    /*(D)--->number-->(E)*/
...
/* 9*/ Trans(Trans::Token,      new TType(TType::Identifier), 10,  0, &IRCre::AddSymbol),      /*(G)----ident--->(H)*/
/*10*/ Trans(Trans::Symbol,     ",",                           9, 11, &IRCre::AddVariable),    /*(H)-----','-----(G)*/
/*11*/ Trans(Trans::Symbol,     ";",                          12,  0, &IRCre::AddVariable),    /*(H)-----';'-----(I)*/
/*12*/ Trans(Trans::Symbol,     "PROCEDURE",                  14, 13, nullptr),                /*(I)->PROCEDURE->(J)*/
/*13*/ Trans(Trans::Nil,        nullptr,                      19,  0, &IRCre::ProcedureStart), /* +--------------(O)*/
/*14*/ Trans(Trans::Token,      new TType(TType::Identifier), 15,  0, &IRCre::AddSymbol),      /*(J)----ident--->(K)*/
/*15*/ Trans(Trans::Symbol,     ";",                          16,  0, &IRCre::AddProcedure),   /*(K)-----';'-----(L)*/
/*16*/ Trans(Trans::GraphStart, &Graph::s_block,              17,  0, nullptr),                /*(L)----block----(M)*/
/*17*/ Trans(Trans::Symbol,     ";",                          18,  0, &IRCre::ProcedureEnd),   /*(M)-----';'-----(N)*/
/*18*/ Trans(Trans::Nil,        nullptr,                      12,  0, &IRCre::RetProcedure),   /*(N)------------>(I)*/
...
{
/* 0*/ Trans(Trans::Token,      new TType(TType::Identifier),  1,  3, &IRCre::BeforeAssignment), /*(A)----ident--->(B)*/
/* 1*/ Trans(Trans::Symbol,     ":=",                          2,  0, nullptr),                  /*(B)-----':='----(C)*/
/* 2*/ Trans(Trans::GraphStart, &Graph::s_expression,         25,  0, &IRCre::AfterAssignment),  /*(C)-expression->(Q)*/
/* 3*/ Trans(Trans::Symbol,     "IF",                          4,  7, nullptr),                  /* +------IF------(D)*/
/* 4*/ Trans(Trans::GraphStart, &Graph::s_condition,           5,  0, &IRCre::Condition),        /*(D)--condition->(E)*/
/* 5*/ Trans(Trans::Symbol,     "THEN",                        6,  0, nullptr),                  /*(E)--->THEN---->(F)*/
/* 6*/ Trans(Trans::GraphStart, &Graph::s_statement,          25,  0, &IRCre::BranchEnd),        /*(F)--statement->(Q)*/
/* 7*/ Trans(Trans::Symbol,     "WHILE",                       8, 11, &IRCre::While),            /* +-----WHILE----(G)*/
/* 8*/ Trans(Trans::GraphStart, &Graph::s_condition,           9,  0, &IRCre::Condition),        /*(G)--condition->(H)*/
/* 9*/ Trans(Trans::Symbol,     "DO",                         10,  0, nullptr),                  /*(H)---->DO----->(I)*/
/*10*/ Trans(Trans::GraphStart, &Graph::s_statement,          25,  0, &IRCre::LoopEnd),          /*(I)--statement->(Q)*/
...
/*16*/ Trans(Trans::Token,      new TType(TType::Identifier), 25,  0, &IRCre::CallProcedure),    /*(L)----ident--->(Q)*/
/*17*/ Trans(Trans::Symbol,     "?",                           18, 19, nullptr),                 /* +------'?'-----(M)*/
/*18*/ Trans(Trans::Token,      new TType(TType::Identifier), 25,  0, &IRCre::InputNumber),      /*(M)----ident--->(Q)*/
/*19*/ Trans(Trans::Symbol,     "!",                          20, 24, nullptr),                  /* +------'!'-----(N)*/
/*20*/ Trans(Trans::Symbol,     "\"",                         21, 23, nullptr),                  /*(N)-----'"'---->(O)*/
/*21*/ Trans(Trans::Token,      new TType(TType::String),     22,  0, &IRCre::OutputString),     /*(O)---string--->(P)*/
/*22*/ Trans(Trans::Symbol,     "\"",                         25,  0, nullptr),                  /*(P)-----'"'---->(Q)*/
/*23*/ Trans(Trans::GraphStart, &Graph::s_expression,         25,  0, &IRCre::OutputNumber),     /* +--expression->(Q)*/
...
/* 1*/ Trans(Trans::GraphStart, &Graph::s_expression, 10, 0, &IRCre::NotEqual),       /*(B)---express-->(E)*/
/* 2*/ Trans(Trans::GraphStart, &Graph::s_expression,  3, 0, nullptr),                /* +----express-->(C)*/
/* 3*/ Trans(Trans::Symbol,     "=",                   9, 4, &IRCre::Equal),          /*(C)----'='----->(D)*/
/* 4*/ Trans(Trans::Symbol,     "#",                   9, 5, &IRCre::NotEqual),       /* +-----'#'----->(D)*/
/* 5*/ Trans(Trans::Symbol,     "<",                   9, 6, &IRCre::Smaller),        /* +-----'<'----->(D)*/
/* 6*/ Trans(Trans::Symbol,     "<=",                  9, 7, &IRCre::LessOrEqual),    /* +-----'<='---->(D)*/
/* 7*/ Trans(Trans::Symbol,     ">",                   9, 8, &IRCre::Larger),         /* +-----'>'----->(D)*/
/* 8*/ Trans(Trans::Symbol,     ">=",                  9, 0, &IRCre::GreaterOrEqual), /* +-----'>='---->(D)*/
/* 9*/ Trans(Trans::GraphStart, &Graph::s_expression, 10, 0, &IRCre::Comparison),     /*(D)---express-->(E)*/
/*10*/ Trans(Trans::GraphEnd,   nullptr,               0, 0, nullptr)                 /*(E)-----------(END)*/
};

std::array<Trans,8> const Graph::s_expression =
{
/* 0*/ Trans(Trans::Symbol,     "-",            2,  1, &IRCre::Negation),    /*(A)----'-'----->(B)*/
...
/* 4*/ Trans(Trans::GraphStart, &Graph::s_term, 3,  0, &IRCre::Addition),    /*(D)----term---->(C)*/
/* 5*/ Trans(Trans::Symbol,     "-",            6,  7, nullptr),             /* +------'-'---->(E)*/
/* 6*/ Trans(Trans::GraphStart, &Graph::s_term, 3,  0, &IRCre::Subtraction), /*(E)----term---->(C)*/
...
/* 3*/ Trans(Trans::GraphStart, &Graph::s_factor, 1, 0, &IRCre::Multiplication), /*(D)---faktor--->(B)*/
/* 4*/ Trans(Trans::Symbol,     "/",              5, 6, nullptr),                /* +-----'/'----->(E)*/
/* 5*/ Trans(Trans::GraphStart, &Graph::s_factor, 1, 0, &IRCre::Division),       /*(E)---faktor--->(B)*/
/* 6*/ Trans(Trans::GraphEnd,   nullptr,          0, 0, nullptr)                 /* +------------(END)*/
};

std::array<Trans,6> const Graph::s_factor =
{
/* 0*/ Trans(Trans::Token,      new TType(TType::Number),     5, 1, &IRCre::ConstByVal),  /*(A)---number--->(D)*/
...
/* 4*/ Trans(Trans::Token,      new TType(TType::Identifier), 5, 0, &IRCre::IdentByName), /* +----ident---->(D)*/
