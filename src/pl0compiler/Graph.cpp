...
/* 2*/ Trans(Trans::Symbol,     ".",             3, 0, &IRCr::ProcedureEnd), /*(C)----'.'---->(D)*/
...
/* 2*/ Trans(Trans::Token,      new TType(TType::Identifier),  3,  0, &IRCr::AddSymbol),      /*(B)----ident--->(C)*/
/* 3*/ Trans(Trans::Symbol,     "=",                           4,  0, nullptr),               /*(C)-----'='-----(D)*/
/* 4*/ Trans(Trans::Token,      new TType(TType::Number),      5,  0, &IRCr::AddConstant),    /*(D)--->number-->(E)*/
...
/* 9*/ Trans(Trans::Token,      new TType(TType::Identifier), 10,  0, &IRCr::AddSymbol),      /*(G)----ident--->(H)*/
/*10*/ Trans(Trans::Symbol,     ",",                           9, 11, &IRCr::AddVariable),    /*(H)-----','-----(G)*/
/*11*/ Trans(Trans::Symbol,     ";",                          12,  0, &IRCr::AddVariable),    /*(H)-----';'-----(I)*/
/*12*/ Trans(Trans::Symbol,     "PROCEDURE",                  14, 13, nullptr),               /*(I)->PROCEDURE->(J)*/
/*13*/ Trans(Trans::Nil,        nullptr,                      19,  0, &IRCr::ProcedureStart), /* +--------------(O)*/
/*14*/ Trans(Trans::Token,      new TType(TType::Identifier), 15,  0, &IRCr::AddSymbol),      /*(J)----ident--->(K)*/
/*15*/ Trans(Trans::Symbol,     ";",                          16,  0, &IRCr::AddProcedure),   /*(K)-----';'-----(L)*/
/*16*/ Trans(Trans::GraphStart, &Graph::s_block,              17,  0, nullptr),               /*(L)----block----(M)*/
/*17*/ Trans(Trans::Symbol,     ";",                          18,  0, &IRCr::ProcedureEnd),   /*(M)-----';'-----(N)*/
/*18*/ Trans(Trans::Nil,        nullptr,                      12,  0, &IRCr::RetProcedure),   /*(N)------------>(I)*/
...
/* 2*/ Trans(Trans::GraphStart, &Graph::s_expression,         25,  0, &IRCr::AfterAssignment),  /*(C)-expression->(Q)*/
/* 3*/ Trans(Trans::Symbol,     "IF",                          4,  7, nullptr),                 /* +------IF------(D)*/
/* 4*/ Trans(Trans::GraphStart, &Graph::s_condition,           5,  0, &IRCr::Condition),        /*(D)--condition->(E)*/
/* 5*/ Trans(Trans::Symbol,     "THEN",                        6,  0, nullptr),                 /*(E)--->THEN---->(F)*/
/* 6*/ Trans(Trans::GraphStart, &Graph::s_statement,          25,  0, &IRCr::BranchEnd),        /*(F)--statement->(Q)*/
/* 7*/ Trans(Trans::Symbol,     "WHILE",                       8, 11, &IRCr::While),            /* +-----WHILE----(G)*/
/* 8*/ Trans(Trans::GraphStart, &Graph::s_condition,           9,  0, &IRCr::Condition),        /*(G)--condition->(H)*/
/* 9*/ Trans(Trans::Symbol,     "DO",                         10,  0, nullptr),                 /*(H)---->DO----->(I)*/
/*10*/ Trans(Trans::GraphStart, &Graph::s_statement,          25,  0, &IRCr::LoopEnd),          /*(I)--statement->(Q)*/
...
/*16*/ Trans(Trans::Token,      new TType(TType::Identifier), 25,  0, &IRCr::CallProcedure),    /*(L)----ident--->(Q)*/
/*17*/ Trans(Trans::Symbol,     "?",                           18, 19, nullptr),                /* +------'?'-----(M)*/
/*18*/ Trans(Trans::Token,      new TType(TType::Identifier), 25,  0, &IRCr::InputNumber),      /*(M)----ident--->(Q)*/
/*19*/ Trans(Trans::Symbol,     "!",                          20, 24, nullptr),                 /* +------'!'-----(N)*/
/*20*/ Trans(Trans::Symbol,     "\"",                         21, 23, nullptr),                 /*(N)-----'"'---->(O)*/
/*21*/ Trans(Trans::Token,      new TType(TType::String),     22,  0, &IRCr::OutputString),     /*(O)---string--->(P)*/
/*22*/ Trans(Trans::Symbol,     "\"",                         25,  0, nullptr),                 /*(P)-----'"'---->(Q)*/
/*23*/ Trans(Trans::GraphStart, &Graph::s_expression,         25,  0, &IRCr::OutputNumber),     /* +--expression->(Q)*/
...
/* 3*/ Trans(Trans::Symbol,     "=",                   9, 4, &IRCr::Equal),          /*(C)----'='----->(D)*/
/* 4*/ Trans(Trans::Symbol,     "#",                   9, 5, &IRCr::NotEqual),       /* +-----'#'----->(D)*/
/* 5*/ Trans(Trans::Symbol,     "<",                   9, 6, &IRCr::Smaller),        /* +-----'<'----->(D)*/
/* 6*/ Trans(Trans::Symbol,     "<=",                  9, 7, &IRCr::LessOrEqual),    /* +-----'<='---->(D)*/
/* 7*/ Trans(Trans::Symbol,     ">",                   9, 8, &IRCr::Larger),         /* +-----'>'----->(D)*/
/* 8*/ Trans(Trans::Symbol,     ">=",                  9, 0, &IRCr::GreaterOrEqual), /* +-----'>='---->(D)*/
/* 9*/ Trans(Trans::GraphStart, &Graph::s_expression, 10, 0, &IRCr::Comparison),     /*(D)---express-->(E)*/
...
/* 4*/ Trans(Trans::GraphStart, &Graph::s_term, 3,  0, &IRCr::Addition),    /*(D)----term---->(C)*/
/* 5*/ Trans(Trans::Symbol,     "-",            6,  7, nullptr),            /* +------'-'---->(E)*/
/* 6*/ Trans(Trans::GraphStart, &Graph::s_term, 3,  0, &IRCr::Subtraction), /*(E)----term---->(C)*/
...
/* 3*/ Trans(Trans::GraphStart, &Graph::s_factor, 1, 0, &IRCr::Multiplication), /*(D)---faktor--->(B)*/
/* 4*/ Trans(Trans::Symbol,     "/",              5, 6, nullptr),               /* +-----'/'----->(E)*/
/* 5*/ Trans(Trans::GraphStart, &Graph::s_factor, 1, 0, &IRCr::Division),       /*(E)---faktor--->(B)*/
...
/* 4*/ Trans(Trans::Token,      new TType(TType::Identifier), 5, 0, &IRCr::IdentByName), /* +----ident---->(D)*/
